...
class UI(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self._key_state = {
            "a": False, "s": False, "d": False, "w": False, "q": False, "e": False,
        ...
        class Overlay(QtWidgets.QWidget):
            ...
            def setHitProgress(self, v: float):
                self.hit_progress = float(v)
            ...
            hitProgress = QtCore.Property(
                float, fget=getHitProgress, fset=setHitProgress, notify=hitProgressChanged)
            ...
            def paintEvent(self, e):
                ...
                p.drawEllipse(QtCore.QPoint(cx, cy), int(
                    size * 0.18), int(size * 0.18))
                # 受击晕影
                if self.hit_progress > 0:
                    ...
                    grad.setColorAt(0.6, QtGui.QColor(
                        255, 50, 50, int(edge_alpha * 0.5)))
        ...
        class HealthBar(QtWidgets.QFrame):
            ...
            def paintEvent(self, e):
                ...
                if self.team == "red":
                    c1, c2, txt = QtGui.QColor(255, 84, 84), QtGui.QColor(
                        210, 50, 50), QtGui.QColor(255, 240, 240)
                elif self.team == "blue":
                    c1, c2, txt = QtGui.QColor(88, 140, 255), QtGui.QColor(
                        48, 90, 210), QtGui.QColor(235, 245, 255)
                else:
                    c1, c2, txt = QtGui.QColor(180, 180, 180), QtGui.QColor(
                        140, 140, 140), QtGui.QColor(240, 240, 240)
                grad = QtGui.QLinearGradient(0, 0, w, 0)
                grad.setColorAt(0.0, QtGui.QColor(
                    c1.red(), c1.green(), c1.blue(), 240))
                grad.setColorAt(1.0, QtGui.QColor(
                    c2.red(), c2.green(), c2.blue(), 240))
                p.setPen(QtCore.Qt.NoPen)
                p.setBrush(QtGui.QBrush(grad))
                p.drawRoundedRect(QtCore.QRectF(
                    1.0, 1.0, fill_w, h - 2.0), r, r)
                ...
                p.drawText(rect, QtCore.Qt.AlignCenter,
                           f"{self.label_text} {self.value:3d}")
                p.end()
        ...
        self.red_bar_top = HealthBar(
            self.top_hud, label_text="红方", team="red", height=64)
        self.blue_bar_top = HealthBar(
            self.top_hud, label_text="蓝方", team="blue", height=64)

        class CountdownBanner(QtWidgets.QFrame):
            def __init__(self, parent=None):
                super().__init__(parent)
                ...
                self.label.setStyleSheet(
                    "color: #f5f7fa; letter-spacing: 1px; margin:0px;")
        ...
        self.top_layout.addWidget(
            self.countdown_banner, 0, QtCore.Qt.AlignVCenter)
        self.top_layout.addSpacing(12)
        ...
        self.exit_btn = QtWidgets.QPushButton(
            "退出", parent=self, objectName="exitBtn")
        self.exit_btn.clicked.connect(self.close)
        self.settings_btn = QtWidgets.QPushButton(
            "设置", parent=self, objectName="settingsBtn")
        self.settings_btn.clicked.connect(self._open_menu)
        self._style_buttons_font()

        # 左下角面板
        self.bottom_left_panel = QtWidgets.QFrame(
            self, objectName="bottomPanel")
        bl = QtWidgets.QVBoxLayout(self.bottom_left_panel)
        bl.setContentsMargins(10, 6, 10, 6)
        bl.setSpacing(0)
        self.self_bar = HealthBar(
            self.bottom_left_panel, label_text="我方", team=None, height=26)
        self.self_bar.setSizePolicy(
            QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Fixed)
        ...
        self.menu_mask.hide()
        self.menu_mask.mousePressEvent = lambda e: self._close_menu()
        ...
        self._applied_video = (self.video_edit.text().strip()
                               if hasattr(self, "video_edit") else None) or None
        self._applied_server = (self.server_edit.text().strip()
                                if hasattr(self, "server_edit") else None) or None

        self._menu_snapshot = None

        # 状态与动画
        self.fps = None
        self.serial_latency = None
    ...
    def set_frame(self, frame_bgr: np.ndarray):
        ...
        qimg = QtGui.QImage(frame_bgr.data, w, h, 3 * w,
        ...
        qimg_scaled = qimg.scaled(
            target, QtCore.Qt.KeepAspectRatioByExpanding, QtCore.Qt.SmoothTransformation)
        ...
        if qimg_scaled.width() > target.width() or qimg_scaled.height() > target.height():
            ...
            # 复制中间部分
            cropped_image = qimg_scaled.copy(
                x_offset, y_offset, target.width(), target.height())
    ...
    def set_color(self, side: str | None):
        self.color = side if side in ("red", "blue") else None
        self.self_bar.set_team(self.color)

    def set_healths(self, red_hp: int, blue_hp: int):
        self.red_bar_top.set_value(int(red_hp))
        self.blue_bar_top.set_value(int(blue_hp))
        self.self_bar.set_value(int(red_hp) if self.color == "red" else int(
            blue_hp) if self.color == "blue" else 0)

    def set_fps(self, fps):
        self.fps = fps
        self._update_status()

    def set_serial_latency(self, ms):
        self.serial_latency = ms
        self._update_status()

    def set_server_latency(self, ms):
        self.server_latency = ms
        self._update_status()

    def get_serial_port(self) -> str | None:
        """
        返回“已应用”的串口设备；只有点确定后才更新。
        """
        return self._applied_serial or None

    def get_video_url(self) -> str | None:
        """
        返回“已应用”的视频流地址；只有点确定后才更新。
        """
        return self._applied_video or None

    def get_server_addr(self) -> str | None:
        """
        返回“已应用”的服务器地址；只有点确定后才更新。
        """
        return self._applied_server or None

    def get_input_packet(self) -> bytes | None:
        """
        返回最新的输入数据包（长度10的bytes）。
        光标显示时返回全零包；若尚未采样也为全零包。
        """
        return self._dbus_packet

    def trigger_hit(self, ts=None):
    ...
    def eventFilter(self, obj, event):
        ...
        if et == QtCore.QEvent.Wheel:
            ...
            if delta:
                self._wheel_accum += float(delta) / 120.0
    ...
    def _map_to_i16(self, val: float, max_val: float) -> int:
    ...
    def _build_input_packet(self, dx: float, dy: float, dz: float, left_pressed: bool, right_pressed: bool, key_state) -> bytes:
        """
        构造10字节数据包（小端序）
        """
        x16 = self._map_to_i16(dx, INPUT_MAX_DX)
        y16 = self._map_to_i16(dy, INPUT_MAX_DY)
        z16 = self._map_to_i16(dz, INPUT_MAX_DZ)
    ...
    def _packet_hex(self, pkt: bytes) -> str:
    ...
    def _sample_input(self):
        ...
        vx = (pos.x() - self._last_mouse_pos.x()) / dt
        vy = (pos.y() - self._last_mouse_pos.y()) / dt
        ...
        # 检查是否接近屏幕边缘
        edge_threshold = 10  # 像素阈值
        if (pos.x() <= screen_rect.left() + edge_threshold or
                pos.x() >= screen_rect.right() - edge_threshold or
                pos.y() <= screen_rect.top() + edge_threshold or
                pos.y() >= screen_rect.bottom() - edge_threshold
                ):
            ...
            self._last_mouse_pos = QtCore.QPoint(center_x, center_y)
        else:
        ...
        self._dbus_packet = self._build_input_packet(
            vx, vy, vz, left_pressed, right_pressed, self._key_state)
        # print(self._packet_hex(self._dbus_packet))
    ...
    def _refresh_serial_ports(self):
        ...
        if list_ports is None:
            ...
            self.serial_combo.addItem("未安装pyserial", "NA")
            return
        ...
        if prev:
            ...
            if idx_prev >= 0:
                ...
                return
        if self._applied_serial:
            idx_applied = self.serial_combo.findData(self._applied_serial)
            if idx_applied >= 0:
    ...
    def _open_menu(self):
        ...
        # 用“已应用”的值回填控件
        self.video_edit.setText(self._applied_video or "")
        self.server_edit.setText(self._applied_server or "")
        if self._applied_serial:
            idx = self.serial_combo.findData(self._applied_serial)
            if idx >= 0:
    ...
    def _apply_menu(self):
        ...
        self._applied_serial = (str(data).strip()
    ...
    def _update_bottom_panel_layout(self):
        if not self._constructed:
            return
        W, H = self.width(), self.height()
        text = self._plain_text(self.status_label.text())
        fm = QtGui.QFontMetrics(self.status_label.font())
        text_w = fm.horizontalAdvance(text) + 18
    ...
    def _update_status(self):
        def seg(label, val, unit):
            ok = val is not None
            color = "#eaeaea" if ok else "#ff5a5a"
            txt = f"{label}: " + (f"{val:02.0f} {unit}" if ok else "未连接")
            return f"<span style='color:{color};'>{txt}</span>"

        fps_txt = seg("视频帧率", self.fps, "fps")
        ser_txt = seg("串口延迟", self.serial_latency, "ms")
        srv_txt = seg("服务器延迟", self.server_latency, "ms")
        html = f"<div style='text-align:center'>{fps_txt}  |  {ser_txt}  |  {srv_txt}</div>"
    ...
    def mousePressEvent(self, e):
        ...
        if not self.menu_panel.isVisible() and self._cursor_shown:
    ...
    def _apply_cursor_state(self):
        enabled = self._cursor_shown
    ...
    def _set_cursor_visible(self, visible: bool):
        self._cursor_shown = visible
        self.setCursor(
            QtCore.Qt.ArrowCursor if visible else QtCore.Qt.BlankCursor)
        self._apply_cursor_state()
        # 重置输入采样参考，并设置全零包
        self._last_mouse_pos = QtGui.QCursor.pos()
        self._last_mouse_time = time.perf_counter()
...
def main():
    ...
    ui.set_healths(red_hp, blue_hp)
    ui.set_countdown(countdown)

    # 状态切换演示
    status_states = [
        {"fps": None, "com": None, "server": None},
        {"fps": 60, "com": 12, "server": 35},
        {"fps": 48, "com": None, "server": 40},
        {"fps": 52, "com": 18, "server": None},
    ]
    status_idx = 0
    ...
    timer_hp.timeout.connect(tick_health)
    timer_hp.start(80)
    ...
    timer_color.timeout.connect(tick_color)
    timer_color.start(4000)

    def tick_print():
        print(ui.get_serial_port(), ui.get_video_url(), ui.get_server_addr())

    timer_print = QtCore.QTimer()
    # timer_print.timeout.connect(tick_print)
